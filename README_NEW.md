...
Note that the apollo cli uses [nodejs](https://nodejs.org) to generate the graphql query files.
...
```java
private static final String BASE_URL = "https://example.com/graphql";
...
Optional<GetBooks.Data> data = Optional.empty();
try {
    data = apolloClient.query(new GetBooks()).execute().data();
    if(data.isPresent()) {
